...
你正在接手 Git clone 后的 rewrite 仓库根目录。不要从旧的 GNN 想法重新开始，也不要恢复已经删除的历史 pilot。当前唯一主计划是：

docs/rewrite_research_plan.md

如果当前是全新目标服务器，必须先完整执行 docs/SERVER_MIGRATION_GUIDE.md。
...
1. 新服务器先完整阅读 docs/SERVER_MIGRATION_GUIDE.md；源服务器外部环境说明只作历史参考。
...
- 活跃代码放在 REWRITE_ROOT；缓存、日志、模型和 raw artifacts 放在目标服务器本机高速盘。
- Python 环境路径由目标服务器自行选择，不属于仓库接口。
- artifact/registry/model/cache 路径统一由 .env.migration 中的环境变量配置。
- 不把高频 I/O 放到慢速共享 home。
- 用户允许自主执行非破坏性工程决策，不要反复询问。
- 删除操作需先征得用户同意；旧 pilot 已按用户要求删除，不要恢复。
...
- 当前迁移改造加入可配置路径、fingerprint v3、自检脚本和服务器迁移手册。
...
- pytest：迁移前 23 passed；迁移改造后 28 passed。
- compileall：通过。
- 12 个 JSON 配置解析通过。
