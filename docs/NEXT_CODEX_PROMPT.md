...
1. 完成 family adapter：
   - validator 使用可注入 input_factory。
   - runner 根据 rewrite_config.family_id 选择 Workload/RMSNormWorkload。
   - baseline、candidate instantiate、input factory 通过 adapter 注入。
   - MLP Phase 1 路径必须保持兼容。
